...
class CATFormer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, in_chans=3, num_classes=80, depths=[3, 3, 3, 3], dims=[64, 128, 320, 512], 
...
#                img_size=img_size, patch_size=4, in_chans=in_chans, embed_dim=dims[0])
    ...
    def forward_features(self, x):
        ...
        x = self.stages[0](x)
        x_out = self.norms[0](x.permute(0, 2, 3, 1).contiguous())
        if 0 in self.out_indices:
            out.append(x_out.permute(0, 3, 1, 2).contiguous())
        x = self.patch_embed2(x)
        x = self.stages[1](x)
        x_out = self.norms[1](x.permute(0, 2, 3, 1).contiguous())
        if 1 in self.out_indices:
            out.append(x_out.permute(0, 3, 1, 2).contiguous())
        x = self.patch_embed3(x)
        x = self.stages[2](x)
        x_out = self.norms[2](x.permute(0, 2, 3, 1).contiguous())
        if 2 in self.out_indices:
            out.append(x_out.permute(0, 3, 1, 2).contiguous())
        x = self.patch_embed4(x)
        x = self.stages[3](x)
        x_out = self.norms[3](x.permute(0, 2, 3, 1).contiguous())
